...
namespace HeritageFabrics.SO
{
    ...
    public class SOOrderEntry_AutoAllocation : PXGraphExtension<SOOrderEntry>
    {
        ...
        private const string CutBinPrefix = "CUT";
            ...
            int inventoryID, int siteID, HashSet<string> assignedSerials)
        {
                ...
                .View.ReadOnly.Select(Base, inventoryID, siteID))
            {
                ...
                // Skip items stored in cut bins (CUT, CUT1A, CUT2B, etc.)
                if (status.LocationID != null && IsExcludedLocation(status.LocationID))
                    continue;
        ...
        /// <summary>
        /// Check if a warehouse location is a cut bin (CUT, CUT1A, CUT2B, etc.).
        /// Cut bins hold remnant pieces that should not be auto-allocated.
        /// Caches lookups to avoid repeated DB hits within the same Persist cycle.
        /// </summary>
        private readonly Dictionary<int, bool> _excludedLocationCache = new Dictionary<int, bool>();

        private bool IsExcludedLocation(int? locationID)
        {
            if (locationID == null) return false;
            int locID = locationID.Value;

            if (_excludedLocationCache.TryGetValue(locID, out bool excluded))
                return excluded;

            INLocation loc = SelectFrom<INLocation>
                .Where<INLocation.locationID.IsEqual<@P.AsInt>>
                .View.ReadOnly.Select(Base, locID);

            string locationCD = loc?.LocationCD?.Trim() ?? "";
            excluded = locationCD.StartsWith(CutBinPrefix, StringComparison.OrdinalIgnoreCase);
            _excludedLocationCache[locID] = excluded;
