...
namespace StudioB.Containers
{
    public class AesthetikContainersInstall : CustomizationPlugin
    {
            ...
            try
            {
                ...
                using (var conn = new SqlConnection(connStr))
                {
                    ...
                    foreach (var kvp in companies)
                    {
                        ...
                        // EnsureDRPGISiteMap removed — DRP GIs are now SQL-installed
                        // with no ScreenID/SiteMap. EnsureContainerTrackingSiteMap
                        // cleans up any stale SiteMap entries from prior publishes.
        ...
        // EnsureDRPGISiteMap removed — DRP GIs are now SQL-installed via
        // project.xml <Sql> blocks with no ScreenID/SiteMap. The old method
        // upserted SiteMap entries and fixed ScreenIDs, which conflicted with
        // the no-ScreenID approach that makes OData work without 403.
